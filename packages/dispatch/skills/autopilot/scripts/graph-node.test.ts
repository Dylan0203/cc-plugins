...
import { type ParsedTask, taskValidity } from "../../flightplan/scripts/lib/parse-task";
import { unmetDependencies } from "../../flightplan/scripts/next-ready";
import { type GraphNode, type NodeValidity, nodesFromParsedTasks, unmetNodeDependencies } from "./graph-node";
...
function node(validity: NodeValidity, dependsOn: string[] = []): GraphNode {
  return {
    ref: "api/01", bucket: "api", nn: "01", title: "API",
    status: "todo", dependsOn, blocks: [], finalReview: false, validity,
  };
}
...
describe("nodesFromParsedTasks", () => {
  test("maps exactly the narrow fields and preserves input keys without mutation", () => {
    ...
    expect(nodesFromParsedTasks({ "input/09": task })).toEqual({
      "input/09": {
        ref: "input/09", bucket: "api", nn: "01", title: "Build API",
        status: "todo", dependsOn: ["core/02"], blocks: ["ui/03"],
        finalReview: true, validity: { kind: "unfinished", status: "todo" },
      },
    });
  ...
  test("passes through complete, unfinished, and invalid validity results", () => {
    const byRef = {
      ...
      "api/04": parsed({ status: "done", body: "## Verification\n\n- [ ] Run the suite\n" }),
    };
...
describe("unmetNodeDependencies", () => {
  test("returns missing, unfinished, and invalid refs in dependency order", () => {
    const task = node({ kind: "unfinished", status: "queued" }, ["missing/01", "pending/01", "bad/01", "done/01"]);
    expect(unmetNodeDependencies(task, {
      "pending/01": node({ kind: "unfinished", status: null }),
      "bad/01": node({ kind: "invalid", rule: "external-source", reason: "Rejected" }),
      "done/01": node({ kind: "complete" }),
    })).toEqual(["missing/01", "pending/01", "bad/01"]);
  });

  test("returns no refs when every dependency is complete", () => {
    expect(unmetNodeDependencies(node({ kind: "unfinished", status: "todo" }, ["done/01", "done/02"]), {
      "done/01": node({ kind: "complete" }),
  ...
  test("an invalid node with no dependencies has no unmet refs", () => {
    expect(unmetNodeDependencies(node({ kind: "invalid", rule: "source", reason: "Bad node" }), {})).toEqual([]);
  });

  test("matches the scout readiness rule for a parsed tree", () => {
    const task = parsed({ dependsOn: ["missing", "unfinished", "invalid", "complete"].map((bucket) => ({ bucket, nn: "01" })) });
    const byRef = {
      "api/01": task,
      "unfinished/01": parsed(),
      "invalid/01": parsed({ status: null }),
      "complete/01": parsed({ status: "done" }),
    };
    const nodes = nodesFromParsedTasks(byRef);
    expect(unmetNodeDependencies(nodes["api/01"], nodes)).toEqual(unmetDependencies(task, byRef));
  });
});
